    ...
    return(
        <div className="grid">
            {Array(rows).fill().map((_,rowIndex) => {
                ...
                const states = isComplete ?getLetterStates(allGuesses[rowIndex], solution) : Array(7).fill('');
                ...
                return (
                    <div key={rowIndex} className="row">
                        {Array(cols).fill().map((_, colIndex) => {
                            ...
                            return (<div key={`${rowIndex}-${colIndex}`} className={`cell cell-${letterState}`}>{letter}</div>);
                        })}  
                    </div>
                )
            })}
